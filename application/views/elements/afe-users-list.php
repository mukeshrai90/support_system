	...
	<tbody>
		...
		?>
			<tr class="">				
				...
				<td><?php echo $user['circle_name']?></td>
				<td><?php echo $user['ssa_name']?></td>
				<td><?php echo $user['afe_address'] ? $user['afe_address'] : 'NA'?></td>
				<td class="tooltip-demo">
					<?php 
						if($user['afe_status'] == 0) {
							echo 
							'<a href="javascript:;" class="change-status" title="Click To Activate" data-toggle="tooltip" data-placement="bottom" data-status="1" data-userid="'.EnCrypt($user['afe_id']).'" data-field="status">
								<i class="fa fa-star-o text-navy" style="color:#FF501E;"></i>&nbsp; (Inactive)
							</a>';
							
						} else if($user['afe_status'] == 1) {
							echo 
							'<a href="javascript:;" class="change-status" title="Click To Deactivate" data-toggle="tooltip" data-placement="bottom" data-status="0" data-userid="'.EnCrypt($user['afe_id']).'" data-field="status">
